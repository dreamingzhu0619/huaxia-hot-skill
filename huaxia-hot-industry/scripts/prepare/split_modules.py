...
# 复用渲染层的 SVG 合法化工具（与 generate_html 同源，保证 modules 存的就是合法 SVG）
...
SLUG_MAP = {
    "头图banner": "banner",
    "热点前线": "hotspot-frontline",
    "市场解读": "market-analysis",
    "推荐理由A": "recommendation-a",
    "推荐理由B": "recommendation-b",
    "买基金 来招行": "buy-fund-cmb",
...
def main():
    ...
    print(f"  D2C: {'有' if d2c_payload else '无'}  顶层片段: {len(d2c_fragments)}"
          f"  页面: {d2c_page.get('width')}x{d2c_page.get('height')}")
